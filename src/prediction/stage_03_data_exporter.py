    ...
    def __init__(self, config, logger, enable_logging):
        try:
            ...
            self.dataset_daily_collection_name = self.config["dataset"][
                "database_detail"]["dataset_daily_prediction_collection_name"]
            self.dataset_hourly_collection_name = self.config["dataset"][
                "database_detail"]["dataset_hourly_prediction_collection_name"]
            self.prediction_file_from_db = self.config["artifacts"][
                'prediction_data']['prediction_file_from_db']
            self.input_csv_day = self.config["artifacts"]['prediction_data']['input_csv_day']
            self.input_csv_hour = self.config["artifacts"]['prediction_data']['input_csv_hour']
    ...
    def export_dataframe_from_database(self):
        try:
            ...
            input_csv_day_file_path = os.path.join(
                self.prediction_file_from_db, self.input_csv_day)
            ...
            #     self.prediction_file_from_db, self.input_csv_hour)
...
def export_main(config_path, enable_logging=True, execution_id=None,
                executed_by=None):
    try:
        logger = get_logger_object_of_prediction(config_path=config_path,
