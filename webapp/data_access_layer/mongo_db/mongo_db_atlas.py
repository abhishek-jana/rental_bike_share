...
import pandas as pd
import sys
from webapp.project_library_layer.credentials.mongo_db_credential import get_mongo_db_credentials
from webapp.exception_layer.data_access_exception.mongo_db_exception import MongoDbException
...
class MongoDBOperation:
    def __init__(self, user_name=None, password=None):
        try:
            if user_name is None or password is None:
                credentials = get_mongo_db_credentials()
                self.user_name = credentials["user_name"]
                self.password = credentials["password"]
                self.url = credentials["url"]
                self.is_cloud = credentials["is_cloud"]
            else:
            ...
            raise Exception(
                mongo_db_exception.error_message_detail(str(e), sys)) from e
    ...
    def get_mongo_db_url(self):
        ...
        """
        try:
            url = self.url.format(urllib.parse.quote_plus(
                self.user_name), urllib.parse.quote_plus(self.password))
    ...
    def get_database_client_object(self):
        """
        return pymongo client to perform actions with MongoDB
        """
        try:
            ...
            mongo_db_exception = MongoDbException(
                "Failed to fetch database client object in module [{0}] class [{1}] method [{2}]"
                .format(MongoDBOperation.__module__.__str__(), MongoDBOperation.__name__, "__init__"))
            raise Exception(
                mongo_db_exception.error_message_detail(str(e), sys)) from e
    ...
    def is_database_present(self, client, db_name):
        ...
        """
        try:
            return bool(db_name in client.list_database_names())
        except Exception as e:
            mongo_db_exception = MongoDbException(
    ...
    def is_collection_present(self, collection_name, database):
        ...
        try:
            ...
            return bool(collection_name in collection_list)
        except Exception as e:
            mongo_db_exception = MongoDbException(
    ...
    def get_collection(self, collection_name, database):
        ...
        """
        try:
            collection = self.create_collection_in_database(
    ...
    def insert_record_in_collection(self, db_name, collection_name, record):
        ...
        try:
            ...
            if not self.is_record_present(db_name, collection_name, record):
                no_of_row_inserted = self.create_record(
    ...
    def insert_records_in_collection(self, db_name, collection_name, records):
        ...
        try:
            ...
            for record in records:
                if not self.is_record_present(db_name, collection_name, record):
                    no_of_row_inserted = no_of_row_inserted + \
                        self.create_record(collection=collection, data=records)
            client.close()
            return no_of_row_inserted
        except Exception as e:
            mongo_db_exception = MongoDbException(
    ...
    def get_min_value_of_column(self, database_name, collection_name, query, column):
        ...
        try:
            ...
            collection = self.get_collection(
                collection_name=collection_name, database=database)
            min_value = collection.find(query).sort(
                column, pymongo.ASCENDING).limit(1)
            value = list(min_value)
    ...
    def get_max_value_of_column(self, database_name, collection_name, query, column):
        ...
        try:
            ...
            collection = self.get_collection(
                collection_name=collection_name, database=database)
            max_value = collection.find(query).sort(
                column, pymongo.DESCENDING).limit(1)
            value = list(max_value)
            if len(value) > 0:
                if column in value[0]:
                    return value[0][column]
            return None
